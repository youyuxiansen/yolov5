...
"""
class AmicroIndoorYolov5Detector(BaseYOLOv5Detector):
    """直接覆盖父类的属性，可以修改默认的锚点和iou值等"""
    _anchors = [[61, 40], [81, 106], [188, 61], [137, 155], [
        112, 316], [201, 228], [396, 210], [251, 368], [441, 362]]
    _names = [ 'feces', 'slippers', 'closetool', 'cabinet', 'curtain',
    ...
    def detect(self, img):
        ...
        src_h, src_w = img.shape[:2]
        new_img, gain = self._preInference(img, self._wh)  # gain is img original size
        result = self._nn.inference(new_img)
        self._afterInference(result, gain, img, src_w, src_h)
    ...
    def _afterInference(self, result, gain, src_img, src_w, src_h):
        ...
        print("BaseYOLOv5Detector -> _afterInference result: ", len(result[0]), len(
            result[0][0]), len(result[0][0][0]), len(result[0][0][0][0]), len(result[0][0][0][0][0]))
        ...
        for t in range(3):
            ...
            grid_h, grid_w, channel_n, predict_n = input0_data.shape
            print(
                "BaseYOLOv5Detector -> _afterInference grid_h / grid_w : ", grid_h, grid_w)
            anchors = [self._anchors[i] for i in self._masks[t]]
            box_confidence = input0_data[..., 4]
            box_confidence = np.expand_dims(box_confidence, axis=-1)
            box_class_probs = input0_data[..., 5:]
            ...
            box_xy -= (box_wh / 2.)  # 计算原尺寸的中心
            box = np.concatenate((box_xy, box_wh), axis=-1)
            res = self.__filter_boxes(
                box, box_confidence, box_class_probs, self._conf_thresh)
            boxes.append(res[0])
            classes.append(res[1])
            scores.append(res[2])
        ...
        for (x, y, w, h), score, cl in zip(boxes, scores, classes):
            ...
            box_list.append((x1, y1, x2, y2))
            if self.draw_box:
                self.__plotOneBox((x1, y1, x2, y2), src_img,
                                  label=self._names[cl])
    ...
    def detect_suit_yolo(self, img):
        ...
        self._calMapAfterInference(result)

    def _calMapAfterInference(self, result):
        print("BaseYOLOv5Detector -> _afterInference result: ", len(result[0]), len(
            result[0][0]), len(result[0][0][0]), len(result[0][0][0][0]), len(result[0][0][0][0][0]))
