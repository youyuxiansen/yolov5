...
def run(
    ...
    if training:  # called by train.py
        ...
        # Load model
        model = DetectMultiBackend(weights, device=device, dnn=dnn, data=data, fp16=half, inference_type=inference_type, model_type=model_type)
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        with dt[2]:
            preds = non_max_suppression(preds, conf_thres, iou_thres, labels=lb, multi_label=True, agnostic=single_cls,
                                        max_det=max_det)
...
def parse_opt():
    ...
    parser.add_argument('--weights', type=str, default=ROOT / 'yolov5s.pt', help='model path(s) (when in am_quanzhi mode, this represents folder that includes the recoginfo.txt file)')
